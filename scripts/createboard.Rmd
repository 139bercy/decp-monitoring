...
plot_ly(stats_cumul, x= ~date, y= ~aife, mode= 'lines', type='scatter', name = labels['aife'], 
        stackgroup = 1, height="100%", width="100%") %>%
  ...
  layout(yaxis=y, xaxis=x, autosize=T, hovermode="closest", legend=list(x=0.1, y=0.95), colorway = mycolors)
```
...
plot_ly(df, labels = ~labels, values = ~values, type = "pie", hole=0.6,
        ...
        text = ~paste(labels, ' : ', values, ' marchés transmis'), showlegend = FALSE, height="100%", width="100%") %>%
...
Ce tableau de bord permet de superviser le processus de construction des données essentielles de la commande publique. Chaque jour, des nouvelles données sont transmises par les différents partenaires.
...
### Partenaires
Un grand merci aux différentes équipes des partenaires qui publient leurs données.
